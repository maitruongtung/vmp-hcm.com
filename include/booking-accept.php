...
acceptStatus;
	...
	if (isset($_POST["save"])) {
		...
		foreach ($makhachhang as $mkh) {
 		...
 		$sql = "UPDATE thuenha SET googlemap = '{$googlemap}', vitri = '{$vitri}', ghichu = '{$ghichu}', loaidat = '{$loaidat}', lydo = '{$lydo}', ketquadauvao = '{$ketquadauvao}', ketquathamdinh = '{$ketquathamdinh}', ketquadamphan = '{$ketquadamphan}', onsite_update = '{$username}' WHERE id = {$id}";
 		//die($sql);
 		//
	 	mysql_query($sql);
